...
-- Rule 2: 앱을 멈추는 NOT NULL 제약은 선택적으로만 사용
...
CREATE TABLE IF NOT EXISTS photos (
  id                UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  trip_id           UUID        REFERENCES trips(id) ON DELETE SET NULL, -- nullable: 미분류 상태 허용
...
COMMENT ON COLUMN photos.trip_id         IS 'nullable: 미분류 사진(classified=false)은 trip_id 없이 저장 가능.';
