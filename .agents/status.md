...
*최종 업데이트: 2026-07-08 by Agent Test*
...
|----------|-------------|-----------|----------------|
| **Agent A (Frontend)** | `TASK_AgentA_007` | ✅ 완료 | 우측 책갈피 및 상단 업로드 버튼 삭제, 헤더 좌측에 뒤로가기(표지로, 목차로) 버튼 추가 연동 |
...
- 백엔드: 미기동 (npm install + .env 설정 후 PORT 4000에서 기동 예정)
...
- [ ] `.env` 환경 변수 입력 (Supabase URL/Key, OpenAI API Key)
- [ ] `SUPABASE_SCHEMA.sql` Supabase 대시보드에 적용
- [ ] `npm install` 및 백엔드 서버(PORT 4000) 기동 확인 (Agent C)
- [ ] 프론트엔드와 백엔드 API 연동 (Agent A, B)
- [ ] 사진 업로드 기능 End-to-End 테스트 (Agent C)
